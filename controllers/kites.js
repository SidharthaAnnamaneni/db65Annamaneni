...
exports.kites_update_Page =  async function(req, res) { 
    console.log("update view for item "+req.query.id) 
    try{ 
        let result = await kites.findById(req.query.id) 
        res.render('kitesupdate', { title: 'Kite Update', toShow: result }); 
    } 
    catch(err){ 
        res.status(500) 
        res.send(`{'error': '${err}'}`); 
...
exports.kites_delete_Page = async function(req, res) { 
    console.log("Delete view for id "  + req.query.id) 
    try{ 
        result = await kites.findById(req.query.id) 
        res.render('kitesdelete', { title: 'Kite Delete', toShow: 
result }); 
    } 
    catch(err){ 
        res.status(500) 
